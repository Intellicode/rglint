...
`packages/plugin/src/rules/graphql-js-validation.ts` (the rule-id mapping at
lines ~14–46).
...
- `rglint-graphql-spec` crate with:
  - `names.rs`: mapping `apollo_compiler::validate` error code → rglint rule
    id, e.g. `FieldsOnCorrectType` → `fields-on-correct-type`. Cover all ~28
    identifiable rules listed in `graphql-js-validation.ts`.
...
- One execution of `apollo_compiler::validate` produces all validation errors;
  the bridge dispatches each to its mapped rule id, so enabling only
  `fields-on-correct-type` filters to just those errors.
- Unmapped error codes (apollo-compiler extras with no graphql-eslint id) →
  surfaced under a synthetic `graphql-spec/unknown` id with a logged warning,
  OR dropped (decide: **drop + log** to avoid noise; revisit if a fixture
  expects one).
...
- A `known-divergences.md` entry per fixture case whose message differs from
  graphql-eslint's (manual audit during port).
...
- Some graphql-eslint spec rule ids may have no apollo-compiler counterpart
  (or vice versa); list both directions in `names.rs` comments.
