...
- `benches/corpora/` — vendored real-world schemas (GitHub, Shopify, etc. —
  pick 2-3 large open schemas; respect their licenses in `deny.toml`).
- Per-rule micro-bench: a small macro generating a `criterion_group` per rule
  over its largest fixture.
...
- Criterion group names are stable (`parse/shopify-schema`, `lint/recommended-gh-schema`).
